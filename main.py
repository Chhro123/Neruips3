    ...
    def on_pretrain_routine_start(self, trainer, pl_module):
        if trainer.global_rank == 0:
            ...
            OmegaConf.save(OmegaConf.create({"lightning": self.lightning_config}),
                ...
                # 非 rank 0 进程等待主进程完成所有文件写入
            if torch.distributed.is_initialized():
                torch.distributed.barrier()
            
            # 重命名日志目录
            if not self.resume and os.path.exists(self.logdir):
                dst_parent, name = os.path.split(self.logdir)
                dst = os.path.join(dst_parent, "child_runs", name)
                # 确保父目录存在
                os.makedirs(os.path.split(dst)[0], exist_ok=True)
                try:
                    os.replace(self.logdir, dst)
...
class ImageLogger(Callback):
    ...
    def log_img(self, pl_module, batch, batch_idx, split="train"):
        ...
        if ((self.check_frequency(check_idx) or split=='val') and 
                hasattr(pl_module, "log_images") and
                callable(pl_module.log_images) and
                self.max_images > 0):
            
            if split=='val' and self.val_count% 25 != 1:
...
if __name__ == "__main__":
    ...
    logdir=os.path.join(opt.logdir, opt.log_folder)
